...
const coreValues: CoreValue[] = [
  {
    ...
    description:
      'In einer digitalen Welt sammeln sich Daten an. Ohne Struktur sind sie nur Ballast. Wir verwandeln statische Informationen in dynamisches Wachstum.',
    color: 'text-red-400',
  },
  {
    ...
    description:
      'Ob CRM, KI-Telefonie oder Webdesign – wir bauen keine Insellösungen. Linkty schafft Ökosysteme, in denen Ihre Prozesse nahtlos ineinandergreifen.',
  ...
  },
  {
    ...
    description:
      'Wenn manuelle Fleißarbeit automatisiert wird und Ihr Außenauftritt überzeugt, hat Ihr Team endlich Zeit für das Wesentliche: Den Abschluss.',
...
const FeatureBlock = ({ value }: FeatureBlockProps) => {
  return (
    <motion.div
    ...
    >
      ...
      <h3 className="text-4xl md:text-5xl font-bold text-white mb-6 leading-tight">{value.title}</h3>
      <p className="text-slate-400 text-lg leading-relaxed max-w-md">{value.description}</p>
    </motion.div>
  );
};
        ...
        <div className="lg:w-1/2 relative z-10 flex flex-col justify-center bg-brand-navy/50 backdrop-blur-sm">
          <div className="min-h-screen flex flex-col justify-center gap-60 py-40 px-8 md:px-20">
            {coreValues.map((value, index) => (
              <FeatureBlock key={index} value={value} />
            ))}
          </div>
        </div>
      </div>
